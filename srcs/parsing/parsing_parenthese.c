...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   parsing_parenthese.c                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: aghalmi <aghalmi@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/02/20 09:05:03 by aghalmi           #+#    #+#             */
/*   Updated: 2026/02/20 09:05:03 by aghalmi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_token	*copy_token_enter(t_token *start, t_token *end, t_list **gc_head_cmd)
{
	t_token	*new_head;
	t_token	*new_current;
	t_token	*current;
	...
	{
		copy = new_token(current->type, current->value, gc_head_cmd);
		if (!copy)
			return (NULL);
		if (!new_head)
			new_head = copy;
...
t_node	*parsing_subshell(t_token *token, t_exec_data *data)
{
	...
	in_token = copy_token_enter(left_paren->next, right_paren,
			&data->gc_head_cmd);
	node = new_node(NODE_SUBSHELL, &data->gc_head_cmd);
	if (!node)
		return (NULL);
