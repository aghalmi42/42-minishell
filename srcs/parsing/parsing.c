...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/02/20 09:09:22 by aghalmi           #+#    #+#             */
/*   Updated: 2026/02/20 09:09:27 by aghalmi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../include/minishell.h"

t_node	*create_redir_node(t_token *redir_token, t_token *right_token,
			t_token *left_token, t_exec_data *data)
{
	t_node	*node;
	t_token	*file;

	node = new_node(NODE_REDIR, &data->gc_head_cmd);
	if (!node)
		return (NULL);
	node->redir_type = redir_token->type;
	process_redir_file(node, right_token, &file, &data->gc_head_cmd);
	if (file)
