...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/02/20 08:48:45 by aghalmi           #+#    #+#             */
/*   Updated: 2026/02/20 08:48:45 by aghalmi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	split_path_pattern(char *input, char **dir_path, char **pattern,
		t_list **gc_head_cmd)
{
	char	*last_slash;
...
int	count_match(char *input, t_list **gc_head_cmd)
{
	...
	if (!dir)
		return (gc_free_one(gc_head_cmd, dir_path),
			gc_free_one(gc_head_cmd, pattern), 0);
	count = 0;
	enter = readdir(dir);
	while (enter)
	{
...
int	process_wildcard_count(char *arg, t_list **gc_head_cmd, t_exec_data *data)
{
	int	count;

	get_match(arg, gc_head_cmd, data);
	if (data->match)
	{
		count = 0;
		while (data->match[count])
			count++;
		return (count);
	}
	return (1);
